...
function emodif_mvpa_decoding_study_noSM(subjNum,maskName,classifier,categories,penalty,shiftTRs,test_phase,rest_shift)
  
  %----------------------------------------------------------------------
  % [stuff] = emodif_mvpa_training_localizer(... ALL ARGS ARE STRINGS ...)
  % * for development -
  % emodif_mvpa_decoding_study_2('105','tempoccfusi_pHg_LOC_combined_epi_space','L2logreg','fsowr', '50','02', 'DFencode', 2)
%   emodif_mvpa_decoding_study_2('104','tempoccfusi_pHg_LOC_combined_epi_space','L2logreg','fsowr', '50','02', 'Preview', 2)
...
%           class_args.lambda = args.penalty;
  ...
  for i = 1:nIterations
      ...
      fid=fopen(fn,'w');
      fprintf(fid,sprintf('%s %s %s %s %s %s %s %s %s %s',args.subjID, args.train_phase, args.test_phase, maskName,  classifier, categories, penalty, shiftTRs,test_phase,rest_shift));
