...
case 1
	...
	chunk_idx = [1:rows(y2_and_y3)/no_chunks:rows(y2_and_y3);rows(y2_and_y3)/no_chunks:rows(y2_and_y3)/no_chunks:rows(y2_and_y3)];
	for i = 1:cols(chunk_idx)
		y2_and_y3_chunk = y2_and_y3(chunk_idx(1, i):chunk_idx(2, i), :);
		disp(sprintf('Saving y2_and_y3_chunk_%d', i))
		save(sprintf('vol_comp/y2_and_y3_chunk_%d', i), 'y2_and_y3_chunk');
	end
	...
	for i = 1:cols(chunk_idx)
		...
		load(sprintf('vol_comp/y2_and_y3_chunk_%d.mat', i)); % loads a y2_and_y3_chunk
		total_entries = zeros(rows(y2_and_y3_chunk), 1);
		parfor j = 1:rows(y2_and_y3_chunk)
...
case 2
	...
	chunk_idx = [1:rows(y2_and_y4)/no_chunks:rows(y2_and_y4);rows(y2_and_y4)/no_chunks:rows(y2_and_y4)/no_chunks:rows(y2_and_y4)];
	for i = 1:cols(chunk_idx)
		y2_and_y4_chunk = y2_and_y4(chunk_idx(1, i):chunk_idx(2, i), :);
		disp(sprintf('Saving y2_and_y4_chunk_%d', i))
		save(sprintf('/jukebox/norman/anataraj/fmri/loci1q/vol_comp/y2_and_y4_chunk_%d', i), 'y2_and_y4_chunk');
	end
	...
	for i = 1:cols(chunk_idx)
		...
		load(sprintf('vol_comp/y2_and_y4_chunk_%d.mat', i)); % loads a y2_and_y4_chunk
		total_entries = zeros(rows(y2_and_y4_chunk), 1);
		parfor j = 1:rows(y2_and_y4_chunk)
...
case 3
	...
	chunk_idx = [1:rows(y3_and_y4)/no_chunks:rows(y3_and_y4);rows(y3_and_y4)/no_chunks:rows(y3_and_y4)/no_chunks:rows(y3_and_y4)];
	for i = 1:cols(chunk_idx)
		y3_and_y4_chunk = y3_and_y4(chunk_idx(1, i):chunk_idx(2, i), :);
		disp(sprintf('Saving y3_and_y4_chunk_%d', i))
		save(sprintf('/jukebox/norman/anataraj/fmri/loci1q/vol_comp/y3_and_y4_chunk_%d', i), 'y3_and_y4_chunk');
	end
	...
	for i = 1:cols(chunk_idx)
		...
		load(sprintf('vol_comp/y3_and_y4_chunk_%d.mat', i)); % loads a y3_and_y4_chunk
		total_entries = zeros(rows(y3_and_y4_chunk), 1);
		parfor j = 1:rows(y3_and_y4_chunk)
