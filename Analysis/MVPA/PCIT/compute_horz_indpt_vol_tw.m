function [] = compute_horz_indpt_vol_tw() %%% analog to JAL's compute_vert_rel_vol.m script.  replacing 'compute_per_branch' code witht he branch logic for horz indpndt 
...
% 		y2 < y1 <= 1, y1 can hold any value that is above the dip (y2)
      ...
      for r = 1:num_y2s 
        ...
        if ~mod(r,floor(0.01*num_y2s)) %this gives out percentage calculated
          fprintf('%d%% ', ceil(100*r/base_num));
        end
        
        base = y(r); %gets content of r (y axis point)
        
        my_y2 = y(y < 0);
        my_y1  = y(y > my_y2); %gets values where y1 > y2
        my_y3 = y(y > 0); % gets values where y3>y2 (with the prior line, this creates a dip)
