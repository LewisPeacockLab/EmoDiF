...
% anonymize_dicm('sourceFolder', 'anonymizedFolder', 3) % use 'sub003'
% 
...
%     'subj_04' specifying both the prefix and starting ID.
% 
% To avoid subject name confusion, the PatientName will be replaced with an ID
...
if ~isstrprop(subID(end), 'digit')
    error('subject ID must be in format of ''sub001''');
end
a = subID;
...
for i = 1:nFile
    ...
    if any(ip)
        ...
        PatientNames{end+1} = pn; %#ok<*AGROW>
        ip = numel(PatientNames);
        subID = sprintf(idStr, ip-1+iPN);
        if mod(numel(subID),2), subID(end+1) = char(0); end
        subjIDs{ip} = subID;
    end
    
    fid = fopen(s.Filename, 'r');
    b8 = fread(fid, inf, '*uint8')';
    fclose(fid);
